...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This function sets up the VL geometry for the POC vehicle.  
%
%Current assumptions:
%   8 motors/4 flaps, and relative placement
%   Wing is symmetric about the body x-z plane
%   Single surface only (no tail surface modeled)
%   No geometric twist (easy to add if needed; theory supports).
%   Single bound vortex line at c/4 point.
%   Cosine panel spacing
%
%   Modifing geometry, for a wing with N segments:
%       sections - 3xN+1 array of LE coordinates of each segment break location
%       (similar to AVL x,y,z definition).  Running from root to tip
%           x1 x2 ... xn+1
%           y1 y2 ... yn+1
%           z1 z2 ... zn+1
%       cs - 1xN array of segment effective chords
%           c1 c2 ... cn
%       Ns - 1xN array of number of h.v. attached to each segment
%           N1 N2 ... Nn
%       g_flap - controls which flap inputs effect which segment (only 1
%                   input per segment currently)
%       g_motor - controls which motor inputs effect which segment (only 1
%                   input per segment currently)
